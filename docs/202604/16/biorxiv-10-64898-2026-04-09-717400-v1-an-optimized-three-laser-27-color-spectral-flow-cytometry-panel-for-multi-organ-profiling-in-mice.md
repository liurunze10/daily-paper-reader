...
tldr: C)模型应用，研究成功追踪了免疫细胞动态并发现了一类新型产糖皮质激素细胞，为标准流式设备下的高维度免疫分析提供了强有力工具。
source: biorxiv
...
motivation: 旨在解决现有小鼠多细胞类型同步分析中高维度流式面板设计困难且缺乏全面性的技术瓶颈。
method: 设计并优化了一套适用于3激光光谱流式细胞仪的27色荧光抗体面板，涵盖免疫与非免疫细胞标志物。
result: 成功在多种小鼠组织中同步鉴定出16种细胞亚群，并利用该面板在炎症模型中发现了新的功能性细胞子集。
conclusion: 该27色面板为在常规3激光流式设备上进行跨器官、高维度的细胞动力学研究提供了高效且可靠的解决方案。
---

## 摘要
尽管高维流式细胞术在解析复杂细胞网络中发挥着关键作用，但由于多重检测固有的难度，目前仍缺乏用于同时分析多种小鼠细胞类型的全面方案。为了填补这一技术空白并解析小鼠模型中的多种细胞群体，我们设计了一套针对三激光光谱流式细胞仪优化的27色流式细胞术方案。该优化方案能够广泛且同时检测来自淋巴系和髓系的16个不同细胞亚群——包括T细胞、B细胞、浆细胞、NK细胞、固有淋巴样细胞、树突状细胞、单核细胞、巨噬细胞、中性粒细胞、嗜酸性粒细胞、嗜碱性粒细胞、肥大细胞——以及非免疫细胞，如上皮细胞、内皮细胞、成纤维细胞和神经元细胞。该方案已成功应用于多种组织，包括脾脏、胸腺、骨髓、外周血、肠系膜淋巴结、腹腔灌洗液、肠上皮和固有层。通过将该方案应用于poly(I:C)模型，我们成功追踪了单核细胞和中性粒细胞群体的动态变化，并利用报告基因表达鉴定出一个此前未被认识的产生糖皮质激素的细胞亚群。该方案将促进在标准三激光流式细胞仪上进行高维免疫表型分析，为剖析不同背景下的细胞动态提供强有力的工具。
