...
tldr: C)模型应用，研究成功追踪了免疫细胞动态并发现了一类新型产糖皮质激素细胞，为免疫学研究提供了高效且普适的工具。
source: biorxiv
...
motivation: 旨在解决现有小鼠多色流式面板在同时检测多种复杂细胞类型及跨器官应用方面的技术局限性。
method: 设计并优化了一套适用于标准3激光光谱流式细胞仪的27色荧光抗体面板，涵盖了免疫与非免疫细胞的多种关键标志物。
result: 该面板成功在多种组织中识别出16种细胞亚群，并利用炎症模型发现了此前未被识别的产糖皮质激素细胞子集。
conclusion: 该27色面板为在常规3激光流式设备上进行高维免疫表型分析提供了强有力的工具，显著提升了对复杂细胞动力学的解析能力。
---
