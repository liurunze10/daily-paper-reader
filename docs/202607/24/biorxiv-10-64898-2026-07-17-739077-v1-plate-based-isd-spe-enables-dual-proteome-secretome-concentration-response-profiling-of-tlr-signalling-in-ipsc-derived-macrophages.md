...
tags: ["query:ros-mp"]
score: 6.0
evidence: iPSC诱导巨噬细胞中TLR信号通路的分析
tldr: 本研究开发了一种基于96孔板的原位消化与固相萃取（ISD-SPE）工作流，解决了质谱分泌组学在药物发现中通量受限的难题。该方法实现了从同一孔中同时分析iPSC衍生巨噬细胞的胞内蛋白质组和胞外分泌组。通过对多种TLR信号通路的分析，该工作流展示了高重现性，成功揭示了受体特异性分泌程序及药物抑制下的浓度-反应关系，为系统级研究炎症反应和药物机制提供了高效工具。
source: biorxiv
...
motivation: 传统的质谱分泌组学分析流程复杂且难以扩展，限制了其在药物发现和高通量免疫信号研究中的应用。
method: 开发了一种基于96孔板的ISD-SPE工作流，结合dia-PASEF技术，实现对同一孔内样本的蛋白质组和分泌组集成分析。
result: 该方法在定量重现性上优于传统沉淀法，并成功识别了不同TLR激活下的特异性分泌模式及药物干预后的浓度响应差异。
conclusion: 该工作流为集成分析胞内信号传导与下游蛋白分泌提供了一种可扩展的策略，有助于深入理解炎症反应和化合物的作用机制。
---
