...
title_zh: 基于孔板的 ISD-SPE 技术实现了 iPSC 衍生巨噬细胞中 TLR 信号传导的蛋白质组-分泌组双重浓度-反应分析
...
tags: ["query:ros-mp"]
score: 6.5
evidence: iPSC来源巨噬细胞中TLR信号通路的分析
tldr: 本研究开发了一种基于96孔板的原位消化与固相萃取（ISD-SPE）工作流，旨在解决质谱分泌组学在药物研发中通量受限的问题。该方法实现了从同一孔中同时提取并分析iPSC衍生巨噬细胞的胞内蛋白质组和胞外分泌组。通过对TLR3、TLR4和TLR7/8信号通路的浓度响应分析，该工作流成功揭示了受体特异性的分泌模式及药物抑制效应，为系统性研究炎症反应和药物作用机制提供了高效、可扩展的工具。
source: biorxiv
...
motivation: 传统的质谱分泌组学分析流程复杂且难以扩展，限制了其在药物发现和高通量信号通路研究中的应用。
method: 开发了一种基于96孔板的ISD-SPE工作流，结合dia-PASEF质谱技术，实现对同一实验孔中蛋白质组和分泌组的集成化处理与分析。
result: 该方法在定量重复性上优于传统沉淀法，并成功识别了不同TLR激活下的特异性分泌程序以及药物抑制在胞内外表现出的效价差异。
conclusion: 该工作流为集成分析胞内信号传导与下游蛋白分泌提供了一种可扩展的策略，有助于深入理解炎症反应及化合物的作用机制。
---

## 摘要
蛋白质分泌是细胞信号传导的关键功能输出，捕捉了塑造免疫行为的刺激和药理扰动的动态反应。在巨噬细胞中，Toll 样受体 (TLRs) 的激活驱动了严格受控的分泌程序，这些程序介导炎症反应并提供通路活性的生物学意义读数。虽然基于质谱 (MS) 的分泌组学能够对这些过程进行无偏见分析，但在药物发现中的广泛应用仍受限于样本制备工作流程的可扩展性。在此，我们描述了一种基于孔板的溶液内消化和固相萃取 (ISD-SPE) 工作流程，该流程能够对条件培养基进行 96 孔处理，从而在同一样本孔中进行蛋白质组和分泌组的整合分析。与基于沉淀的方法进行的基准测试表明，该方法具有相当的蛋白质组深度，同时提高了定量重现性，并在多个孔板上表现出稳健的性能。结合 dia-PASEF 采集技术，该工作流程实现了对巨噬细胞对 TLR 激活反应的深入分析，解析了 TLR3、TLR4 和 TLR7/8 激活后的受体特异性分泌程序。将该方法扩展到浓度-反应研究，实现了对细胞内和细胞外蛋白质景观中药理扰动的定量表征，揭示了对 TLR 抑制的共同和室室特异性反应，以及与分泌动力学相关的表观效力差异。总之，该工作流程为细胞内信号传导和下游蛋白质分泌的整合分析提供了一种可扩展的策略，能够实现炎症反应和化合物作用机制的系统级表征。
