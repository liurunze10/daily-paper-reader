...
tags: ["query:ros-mp"]
score: 7.0
evidence: 分析iPSC衍生巨噬细胞中的TLR信号和炎症反应
tldr: 本研究开发了基于96孔板的原位消化与固相萃取（ISD-SPE）工作流，解决了质谱分泌组学在药物发现中扩展性不足的难题。该方法可从同一孔中同步分析iPSC衍生巨噬细胞的胞内蛋白质组与胞外分泌组。通过对多种TLR信号通路的深度刻画，该流程成功解析了受体特异性分泌程序及药物干预的浓度-反应关系，为系统级研究炎症反应和药物机制提供了高效工具。
source: biorxiv
...
motivation: 传统的质谱分泌组学分析流程复杂且难以扩展，限制了其在药物发现和大规模信号通路研究中的应用。
method: 开发了一种基于96孔板的板载原位消化与固相萃取（ISD-SPE）工作流，支持从单一样本孔中同步处理蛋白质组和分泌组。
result: 该方法在保持蛋白质组深度的同时提升了定量重复性，并成功解析了不同TLR激活下的特异性分泌模式及药物干预的浓度效应。
conclusion: 该工作流为整合分析胞内信号传导与下游蛋白分泌提供了一种可扩展的策略，有助于深入理解炎症反应及化合物的作用机制。
---
